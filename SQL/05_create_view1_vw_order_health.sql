-- Creating the views now with both the queries here. One without the where clause and one with it
-- Views are better for MVP: Easier to create, easier to update,  no need to rerun table, if raw data changes, the view reflects it immediately
CREATE OR REPLACE VIEW `ferrous-biplane-450410-i2.AI_Mockproject.vw_order_health`
AS
WITH
  base_calc AS (
    SELECT
...
SELECT
  *,
  CASE
    ...
    WHEN low_margin_flag = 1 OR critical_delay_flag = 1 THEN "Medium"
    ELSE "Low"
